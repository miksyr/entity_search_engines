...
class TestGoogleEntityEngine(TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.googleEngine = GoogleEntityEngine(
            apiKey=os.environ["GOOGLE_SEARCH_API_KEY"]
        )
    ...
    def test_search_entity(self):
        returnEntities = self.googleEngine.search_for_entity(
            query=EXAMPLE_ENTITY.entityName, types=[EXAMPLE_ENTITY.entityType]
        )
    ...
    def test_search_entity_invalid(self):
        noResults = self.googleEngine.search_for_entity(
